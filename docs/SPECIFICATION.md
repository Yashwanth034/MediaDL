...
## Installation
One-time install. The end user should not need to activate a virtual environment, `cd` into the repository, start a service, or reinstall for every use. Release artifacts target Linux x86_64/ARM64, Windows x86_64/ARM64, macOS Intel, and macOS Apple Silicon.

## External components
- yt-dlp: extraction/download engine, integrated behind an adapter
- JavaScript runtime/EJS: standalone builds bundle a checksum-verified supported Deno runtime and yt-dlp EJS for modern YouTube challenge solving. Source/pip installs may use supported Deno, Node 22+, QuickJS, or Bun; YouTube operations fail fast when no supported runtime/EJS is available rather than misclassifying extractor breakage as media unavailability
...
- Standalone installs check the official latest MediaDL release manifest at most once every six hours and install newer checksum-verified MediaDL builds automatically; failed/offline checks never block normal commands.
- Python packages and GitHub Actions are monitored daily by Dependabot. Updates are proposed as pull requests rather than blindly merged.
- The bundled Deno version is checked daily against Deno's official stable GitHub release and updated through an automated pull request when newer.
- Dependency pull requests run the full Linux x86_64/ARM64, Windows x86_64/ARM64, macOS Intel, and macOS Apple Silicon release-build matrix before they are suitable for release.
- FFmpeg/ffprobe remain operating-system-managed external tools and are never silently replaced by MediaDL.
